...
- **Note:** The underflow/overflow logic is identical to that of an unsigned value with fewer bits (e.g., UINT).
- The 64-bit value is made human-readable on the PLC by cyclically converting it to text (variable sUint64).
...
- **Note:** The underflow/overflow logic is identical to that of a signed value with fewer bits (e.g., INT).
- The 64-bit value is made human-readable on the PLC by cyclically converting it to text (variable sInt64).

**Publishing as 64-bit value on the OpcUa server:**
- On the PLC, 64-bit values are structured as BrbUint64_TYP or BrbInt64_TYP.
- To configure the server to publish this structure as Uint64 or Int64, a simple configuration is required.
  - This must be done in the file 'OpcUaMap.uad' (see ConfigurationView/Connectivity/OpcUa), specifically in the 'UA Datatype' column of the structure variable (sub-elements of the structure should not be released).
  - An OpcUa data type NodeId can optionally be specified:
    - **Uint64:** ns=0;i=9
    - **Int64:** ns=0;i=8
- The server will then publish the variable value as the specified OpcUa data type.
- **Note:** This only works if the PLC variable has the byte length required by the specified OpcUa type.
  - OpcUa data types Uint64 and Int64 each require 8 bytes, which are provided by the instances of the structures BrbUint64_TYP and BrbInt64_TYP.
